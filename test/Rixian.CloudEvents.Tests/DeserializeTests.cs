...
namespace Rixian.CloudEvents.Tests
{
    public class DeserializeTests
    {
        ...
        {
            var json = File.ReadAllText($@".\samples\json\{fileName}");
        ...
        {
            var json = File.ReadAllText($@".\samples\binary\{fileName}");
            var evnt = CloudEventV0_1.Deserialize(json);
        ...
        {
            var json = File.ReadAllText($@".\samples\string\{fileName}");
            var evnt = CloudEventV0_1.Deserialize(json);
        ...
        {
            var json = File.ReadAllText($@".\samples\none\{fileName}");
            var evnt = CloudEventV0_1.Deserialize(json);
